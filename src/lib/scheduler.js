...
const { notify, ledgerWriterIds } = require('./auth');
...
function checkBudgets() {
  ...
  for (const b of budgets) {
    ...
    if (get('SELECT id FROM notifications WHERE body = ? LIMIT 1', dedupeKey)) continue;
...
function runDaily() {
  const r = { recurring: 0, budgets: 0, debts: 0, goals: 0 };
...
function initScheduler() {
  // 启动时先跑一次（补上次停机期间漏掉的周期账单）
  setTimeout(() => {
    const r = runDaily();
    if (r.recurring || r.budgets || r.debts || r.goals) {
      console.log(`[scheduler] 启动检查完成：周期账单 ${r.recurring} 笔 / 预算预警 ${r.budgets} / 借贷提醒 ${r.debts} / 目标提醒 ${r.goals}`);
    }
  }, 3000).unref?.();
...
module.exports = { initScheduler, runDaily, runDueRecurring, checkBudgets, checkDebts, checkGoals, budgetPeriodRange, budgetUsedInRange, budgetUsed, advanceDate };
