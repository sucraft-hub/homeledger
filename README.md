...
**怎么知道有哪些模型可选？** 直接问接口要列表（把地址和 Key 换成你自己的）：

```bash
curl -s "$BASE_URL/models" -H "Authorization: Bearer $API_KEY" | grep -o '"id":"[^"]*"'
```

同一个网关（如 one-api / LiteLLM / Ollama）通常挂了多个模型，名称原样填进「模型名称」即可。**换了接口地址就要跟着换模型名。**
